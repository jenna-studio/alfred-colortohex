...
with open("/tmp/alfred_debug_query.txt", "w") as f:
...
for name, hexcode in color_map.items():
    if query in name.lower():
        ...
        if not os.path.exists(icon_path):
            ...
            draw.ellipse((0, 0, 128, 128), fill=hexcode)
            img.save(icon_path)

        items.append({
            "title": hexcode,
            "subtitle": f"Copy HEX for '{name}'",
            "arg": hexcode,
            "icon": {"path": f"icons/{name}.png"}
        })
...
if not items:
    items.append({
        ...
        "subtitle": f"No HEX code for '{query}'",
        "arg": ""
    })
